...
#    i=++i
        for fs in json_fs:
            ...
            if json_fs[fs]['key_tag']  not in res2:
             # print json_fs[fs];
                 arr_tr=[0,0];
                 tradepair=json_fs[fs]['tradepair']
                 tradepair_key=tradepair.split(":")[1]
                 if tradepair_key not in res3:
                     profits_key= (float(json_fs[fs]['equivalentprice']) - float(json_fs[fs]['reverseprice']))*int(json_fs[fs]['volume'])
                     arr_tr[0]=1
                     arr_tr[1]=profits_key
                     res3[tradepair_key]=arr_tr
                 else :
                     arr_tra=res3[tradepair_key]
                     profits_key= (float(json_fs[fs]['equivalentprice']) - float(json_fs[fs]['reverseprice']))*int(json_fs[fs]['volume'])
                     arr_tra[0]=arr_tra[0]+1
                     arr_tra[1]=arr_tra[1]+profits_key
             #交易次数增加实现 
                 num_all=num_all+1
                 #利润增加实现   
...
for key,value in res3.items():
    print key.decode('utf-8').encode('gb18030')+"个单词出现次数:"+str(value[0]) +" 对应的profits： "+ str(value[1])
#print type(res)
print ''
#print list2
print len(res2)
